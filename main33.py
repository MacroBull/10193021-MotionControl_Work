...
def calPos(x,y): # Convert centimeter coord to servo position value, models provided by ***
	powx=x**2
	powy=y**2
...
while 1: # Main loop
	...
	#inr = cv2.inRange(hsv, (100,200,100), (150,155,130))
	#inr = cv2.inRange(hsv, (90,40,180), (140,255,255)) #blue
	inr = cv2.inRange(hsv, (90,90,00), (140,255,255)) #blue
	#inr = cv2.inRange(img, (220,20,20), (255, 255,200)) # blue
	#inr = cv2.inRange(img, (0,0,0), (40, 40,40)) #black
	...
	h = 0
	for c in cont:
		if len(c)> 50: # The really large polygon is what I want
			l = 0.
			r = 1.
			while (h != 4) and (r - l > 0.01): # Binary search for h = 4, simplify to a quadrangle
				m = (l + r) / 2
				approx = cv2.approxPolyDP(c, m*cv2.arcLength(c, True), True)
				h = len(approx)
				if h > 4 :
					l = m
				else:
	...
	if h == 4: # Success to find a quadrangle
		...
		build2(xmap, ymap, SCR_LEN, SCR_WID, p0[0], p0[1], p1[0], p1[1], p2[0], p2[1], p3[0], p3[1], 0) # Implemented in macrobull/projCorrection/core.c for performance, model provided by ***
		timeElps("Built:")
